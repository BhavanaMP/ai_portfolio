...
from losses.uce_loss import CELoss
...
class MCDTrainer:
    def __init__(self, args, gpu_id):
        ...
        self.hparams = {
            ...
            "vanilla": not args.train_mcd, "mcd": args.train_mcd, "forward_passes": args.forward_passes, "ohem": args.use_ohem,
        ...
        self.model = MCDClassifier(num_classes=self.num_classes, encoder=args.encoder, decoder=args.decoder, train_mcd=self.train_mcd, forward_passes=self.forward_passes, device=self.device)
        ...
        self.optimizer = AdamW(
             params=[{"params": self.model.model.encoder.parameters(), "lr": self.initial_lr},
                     ...
                     ], lr=self.initial_lr, weight_decay=1e-4)
        # Scheduler
        ...
        if "baseline" in args.name.lower():
            ...
            self.model_ckpt_path = Path(f"{args.mcd_ckpt_save_loc}", f"model_{args.name}_{self.dataset_name}_{args.run_id[-7:]}.pt").absolute().__str__()
        ...
        # Loss
        self.criterion = CELoss(id2label=self.id2label, use_uncertainty_weighting=self.train_mcd, use_ohem=self.use_ohem, ignore_index=self.ignore_index, device=self.device).to(self.device)
        ...
        if self.is_main_process():
            ...
            # # Start TensorBoard
            # self.tb_process = subprocess.Popen(["tensorboard", "--logdir", self.log_dir, "--port", "8008"], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
            # print(f"[INFO]: Started TensorBoard with logdir {self.log_dir}")
    ...
    # def __del__(self):
    #     """
    #     Set up an SSH tunnel from your local machine to the SLURM node:
    #         ssh -L 8008:localhost:8008 your_username@remote_host
    #         Step 4: Access TensorBoard in Your Browser
    #     Once the SSH tunnel is established, open your web browser and go to: http://localhost:8008
    #     to view tensorboard logs
    #     """
    #     if self.is_main_process():  # Only the master process terminates TensorBoard
    #         self.tb_process.terminate()
    #         self.tb_process.wait()
    #         print("[INFO]: TensorBoard process terminated")
    ...
    def train_step(self, train_dataloader, epoch):
        ...
        for step, batch in enumerate(tqdm(train_dataloader)):
            ...
            with autocast():
                if self.train_mcd:
                    logits, predictive_uncertainty = self.model(images)
                    print(logits.shape, predictive_uncertainty.shape, gt_masks.shape, gt_masks.squeeze(dim=1).long().shape)
                    train_loss = self.criterion(logits, gt_masks.squeeze(dim=1).long(), predictive_uncertainty)
                else:
                    logits = self.model(images)
                    train_loss = self.criterion(logits, gt_masks.squeeze(dim=1).long())
            print(f"\nTrain Step: {step}, Train Loss: {train_loss.item():.3f}")
            # Get the pred mask from logits
    ...
    def val_step(self, val_dataloader, epoch):
        ...
        for step, batch in enumerate(tqdm(val_dataloader)):
            ...
            with torch.no_grad():
                if self.train_mcd:
                    val_logits, predictive_uncertainty = self.model(images, is_val=True)
                    val_loss = self.criterion(val_logits, gt_masks.squeeze(dim=1).long(), predictive_uncertainty)
                else:
                    val_logits = self.model(images, is_val=True)
                    val_loss = self.criterion(val_logits, gt_masks.squeeze(dim=1).long())
            print(f"\nVal Step: {step}, Val Loss: {val_loss.item():.3f}")
            # Get the pred mask from logits
            probs = F.softmax(val_logits, dim=1)
            preds = torch.argmax(probs, dim=1).to(self.device)
            # Metrics calculation per batch
            running_val_loss += val_loss.item()
            self.val_metrics.update(preds.long(), gt_masks.squeeze(dim=1).long())
            if len(selected_data) <= 1:
                idx = 0  # get the first idx of batches per step
                selected_data.append((images[idx].squeeze(), gt_masks[idx].squeeze(), preds[idx].squeeze()))
            del batch, val_logits
    ...
    def train(self):
        ...
        for epoch in tqdm(range(self.start_epoch, self.epochs)):
            ...
            if self.is_main_process():
                print(f"\nEpoch : {self.start_epoch} | Train Loss: {train_metrics_epoch['loss']} | Train mIoU: {train_metrics_epoch['mean_iou']} | Val Loss: {val_loss} | Val mIoU: {val_metrics_epoch['mean_iou']} | Val mAcc: {val_metrics_epoch['mean_accuracy']} | \n")
            if self.is_distributed_mode():
