...
def load_obstacles(split="data", path="BhavanaMalla/obstacles_track"):
    dataset = load_dataset(path, split=split)
    return dataset


def get_obstacles_labels():
    """
...
def get_obstacles_test_transforms():
    # We use full size
...
class CustomObstaclesDataset(torch.utils.data.Dataset):
    def __init__(self, dataset, split="data", crop_size=1024, ignore_index=255):  # 768
        """
        Load obstacles from HF dataset.
