...
def load_anamoly_test_dataset(split="data", path="BhavanaMalla/anamoly_track"):
    dataset = load_dataset(path, split=split)
    return dataset


def get_anamoly_labels():
    """
...
def get_anamoly_test_transforms():
    # We use full size
...
class CustomAnamolyDataset(torch.utils.data.Dataset):
    def __init__(self, dataset, split="data", crop_size=1024, ignore_index=255):  # 768
        """
        Load anamoly dataset from Hugging Face.
