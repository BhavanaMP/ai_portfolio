...
with open(word_file,encoding="utf8") as infile:
    topic = random.choices(list(open(word_file,encoding="utf8")),k=20) #choosing random items from the text file into a list
...
for i in range(len(topic)):
 #this is the config for to get the first introduction of a title
    ...
    try: #catching the file not found error
...
#now that we havae the number of images in the page, we ask for the images that are in the page with the title
    ...
    #image_page is a dictionary with few keys where as image key is one among them that holds image info
